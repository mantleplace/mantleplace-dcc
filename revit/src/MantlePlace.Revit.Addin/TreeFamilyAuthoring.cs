...
/// <summary>
/// The developer command that authors <c>Mantle Place Tree.rfa</c> from the installed Revit's
/// Planting template, measures it, and saves it for committing. Not product: no button, no curator
/// ever reaches it.
/// </summary>
/// <remarks>
/// <para>
/// Set <see cref="DirectoryVariable"/> to a folder and start Revit <b>2025</b>; once Revit has
/// initialised, this writes <see cref="TreeFamily.FileName"/> and <see cref="LogFileName"/> into that
/// folder and changes nothing else. 2025 because a family saved by a Revit loads in that Revit and
/// every later one, never an earlier one — the same floor the shim compiles against. The file it
/// writes is committed as <c>Families/MantlePlaceTree.rfa</c> beside this source and embedded in
/// the assembly; see <c>revit/README.md</c> for the whole loop.
/// </para>
/// <para>
/// It is code rather than a click-by-click recipe so that the family is a function of this file:
/// its parameters, formulas and proportions are <see cref="TreeFamily"/>'s, the same constants the
/// DirectShape fallback builds with, and a change to them is a re-run rather than a re-draw.
/// </para>
/// <para>
/// ⛔ <b>It is not finished until the log says the measurements agree.</b> Two flexes in the family
/// document and one placed instance in a scratch project are measured against the numbers that
/// drove them. A family whose dimension labels did not take would still save, still load and still
/// place — every tree at the template's default size — so the measurement is the only proof.
/// </para>
/// </remarks>
internal static class TreeFamilyAuthoring
{
    /// <summary>The folder the family and its log are written to. Unset, nothing happens.</summary>
    internal const string DirectoryVariable = "MANTLEPLACE_AUTHOR_TREE_FAMILY";

    internal const string LogFileName = "Mantle Place Tree.authoring.log";
    ...
    /// <summary>The type's defaults: a tree a curator would recognise if they placed one by hand.</summary>
    private const double DefaultHeightM = 10.0;
    private const double DefaultCrownRadiusM = 3.0;
    ...
    /// <summary>Authors, measures and saves the family. Always writes the log, whatever happened.</summary>
    /// <returns>Whether the family was saved and every measurement agreed.</returns>
    internal static bool Run(RevitApplication application, string outputDirectory)
    {
        ArgumentNullException.ThrowIfNull(application);

        StringBuilder log = new();
        string familyPath = Path.Combine(outputDirectory, TreeFamily.FileName);
        bool agreed = false;
        ...
        try
        {
            ...
            log.AppendLine(CultureInfo.InvariantCulture,
                $"Mantle Place {PluginVersion.Current} tree family authoring, Revit {application.VersionNumber} ({application.VersionBuild}).");
            ...
            try
            {
                ...
                Author(family, log);
                // Taller than the default with the same crown, then smaller in both — so a label tied
                // to the wrong parameter shows — then back to the default, which is what the type
                // saves with. `&`, not `&&`: every flex runs and is logged even after one disagrees.
                agreed = Flex(family, 12.0, DefaultCrownRadiusM, log)
                    & Flex(family, 4.0, 1.25, log)
                    & Flex(family, DefaultHeightM, DefaultCrownRadiusM, log);
                Purge(family, log);
            ...
            agreed &= VerifyPlacement(application, familyPath, log);
        }
        ...
        log.AppendLine(agreed
            ? "RESULT: every measurement agreed. The family is ready to commit."
            : "RESULT: NOT READY. Something above did not agree; do not commit this family.");
        try
        {
            File.WriteAllText(Path.Combine(outputDirectory, LogFileName), log.ToString());
        }
    ...
    /// <summary>
    /// Adds the parameters, draws a trunk and a crown, and ties every dimension of both to them.
    /// </summary>
    private static void Author(Document family, StringBuilder log)
    {
        ...
        {
            manager.NewType(TreeFamily.FamilyName);
        }

        FamilyParameter height = InstanceLength(manager, TreeFamily.HeightParameter, log);
        FamilyParameter crown = InstanceLength(manager, TreeFamily.CrownRadiusParameter, log);
        manager.Set(height, Internal(DefaultHeightM));
        manager.Set(crown, Internal(DefaultCrownRadiusM));
        ...
        double defaultTrunkHeight = Internal(DefaultHeightM * TreeFamily.TrunkHeightFraction);

        Extrusion trunk = family.FamilyCreate.NewExtrusion(
            true,
            Profile(Circle(0.0, Internal(DefaultCrownRadiusM * TreeFamily.TrunkRadiusFraction))),
            plane,
            defaultTrunkHeight);
        ...
        Blend crownForm = family.FamilyCreate.NewBlend(
            true,
            Circle(Internal(DefaultHeightM), Internal(DefaultCrownRadiusM * TreeFamily.CrownApexFraction)),
            Circle(0.0, Internal(DefaultCrownRadiusM)),
            plane);
    ...
    private static bool Flex(Document family, double heightM, double crownRadiusM, StringBuilder log)
    {
        FamilyManager manager = family.FamilyManager;
        using Transaction transaction = new(family, "Flex the Mantle Place Tree");
        transaction.Start();
        manager.Set(manager.get_Parameter(TreeFamily.HeightParameter), Internal(heightM));
    ...
    /// <summary>
    /// Loads the saved family into a scratch project, places one tree through <see cref="TreeInstances"/>
    /// — the tree step's own code — and measures it.
    /// </summary>
    private static bool VerifyPlacement(RevitApplication application, string familyPath, StringBuilder log)
    {
        Document project = application.NewProjectDocument(Autodesk.Revit.DB.UnitSystem.Metric);
        try
        {
            using Transaction transaction = new(project, "Place a Mantle Place Tree");
            transaction.Start();
    ...
    private static void Purge(Document family, StringBuilder log)
    {
        int purged = 0;
        for (int pass = 0; pass < 5; pass++)
        {
            ...
            using Transaction transaction = new(family, "Purge the Mantle Place Tree");
            transaction.Start();
            int before = purged;
