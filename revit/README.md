...
and no second build is started (`HPS-24`). A Prepare is picked back up for 7 days after you pressed
it, only under the account that pressed it, and only by a Revit that is not already watching it; an
order no longer in your vault, or no longer available, is dropped without a notice. The record is
