...
a case carrying its own `manifestVersion` — `manifest.locationBlockIgnored` is the first and
`manifest.foliageVocabularyIgnored` the second, `manifest.hostFramesIgnored` the third, `manifest.hazardsAndContoursIgnored` the fourth. Stamping a newer version onto a fixture that lacks
what that version requires would make the fixture untrue.
